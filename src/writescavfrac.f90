subroutine writescavfrac(itime, j, ks)
  ...
  if (itime.eq.0) then
      open(unitscav,file=path(2)(1:length(2))//outname,form='formatted',err=998)
  else
      open(unitscav,file=path(2)(1:length(2))//outname,ACCESS='APPEND',form='formatted',err=998)
  endif
  ...
  write(unitscav,*)  adate, atime, itime, j, ks, xscav_frac1(j,ks)  
